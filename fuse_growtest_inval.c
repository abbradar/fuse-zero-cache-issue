...
static _Atomic size_t last_inval_size = INIT_SIZE;
...
static void gt_getattr(fuse_req_t req, fuse_ino_t ino,
  ...
  size_t old = atomic_load(&last_inval_size);

  /* Invalidate the growth region BEFORE replying with the new size.
   * The notify is processed synchronously inside the kernel write(),
   * so stale pages are gone before the i_size update from our reply. */
  if (sz > old) {
    off_t off = old & ~(off_t)(4096 - 1);
    fuse_lowlevel_notify_inval_inode(fuse_session, FILE_INO, off, 0);
    atomic_store(&last_inval_size, sz);
  }
